...
@Tag(name = "Notification Settings (알림 설정)", description = "알림 설정 및 발송 API")
public interface NotificationSettingControllerDocs {

    @Operation(summary = "알림 설정 조회 api", description = "사용자의 모든 알림 설정을 조회합니다. 처음 진입 시 FOLLOW 설정이 없으면 자동으로 생성됩니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "조회 성공")
    })
    whoreads.backend.global.response.ApiResponse<NotificationResDTO.TotalSettingDTO> getNotificationSettings(
    ...
    @Operation(summary = "알림 설정 생성 api", description = "알림 설정을 생성합니다. <br>" +
            "1. **FOLLOW**: `type` 필수. 데이터가 이미 있으면 기존 설정을 업데이트합니다." +
            "(팔로우는 조회때 생성되기 때문에 생성할 필요 없음)<br>" +
            "2. **ROUTINE**: `type`, `days`, `time` 필수." +
            "day : MONDAY, TUESDAY,WEDNESDAY,THURSDAY,FRIDAY,SATURDAY,SUNDAY 또는 0~6")
    @ApiResponses({
    ...
    whoreads.backend.global.response.ApiResponse<NotificationResDTO.SettingDTO> createNotificationSetting(
            @AuthenticationPrincipal Long memberId,
            @RequestBody @Valid NotificationReqDTO.SettingDTO createSettingDTO
    );

    @Operation(summary = "알림 설정 수정 api", description = "기존 알림 설정을 수정합니다. 본인의 알림만 수정 가능하며, 타입(FOLLOW/ROUTINE) 변경은 불가합니다.")
    @ApiResponses({
    ...
    whoreads.backend.global.response.ApiResponse<NotificationResDTO.SettingDTO> updateNotificationSetting(
            @AuthenticationPrincipal Long memberId,
    ...
    @Operation(summary = "알림 설정 삭제 api", description = "설정된 알림을 삭제합니다. 본인의 알림만 삭제 가능합니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "삭제 성공"),
            @ApiResponse(responseCode = "403", description = "삭제 권한 없음 (본인의 알림이 아님)"),
            @ApiResponse(responseCode = "404", description = "알림 존재하지 않음")
    })
    whoreads.backend.global.response.ApiResponse<Void> deleteNotificationSetting(
            @AuthenticationPrincipal Long memberId,
